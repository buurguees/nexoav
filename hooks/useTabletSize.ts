...
/**
 * Hook para detectar el tamaño de pantalla tablet portrait
 ...
 */
export function useTabletSize(): 'small' | 'medium' | 'large' {
  // Obtener el ancho del sidebar del contexto - esto se actualiza automáticamente cuando cambia
  let sidebarWidth = 160; // Valor por defecto (expandido)
  try {
  ...
  const calculateSize = (width: number, sidebar: number): 'small' | 'medium' | 'large' => {
    ...
    if (availableWidth < 500) return 'small';      // < 500px disponible: Tablet pequeño
    if (availableWidth < 700) return 'medium';     // 500px - 699px: Tablet medio
    return 'large';                                 // >= 700px: Tablet grande
  };
