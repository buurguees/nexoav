...
export function ExternosDesktop() {
  ...
  // Componente interno para el listado de externos
  function ExternosListContent() {
    const [suppliers, setSuppliers] = useState<SupplierData[]>([]);
    const [isLoading, setIsLoading] = useState(true);

    // Cargar datos de proveedores externos
    useEffect(() => {
      const loadSuppliers = async () => {
        try {
          setIsLoading(true);
          const data = await fetchSuppliers("externo");
          setSuppliers(data);
        } catch (error) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Listado de proveedores externos - Ocupa 60% del espacio */}
        <div
          style={{
            display: "flex",
            flexDirection: "column",
