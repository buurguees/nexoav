...
export function SoftwaresDesktop() {
  ...
  // Componente interno para el listado de softwares
  function SoftwaresListContent() {
    const [suppliers, setSuppliers] = useState<SupplierData[]>([]);
    const [isLoading, setIsLoading] = useState(true);

    // Cargar datos de proveedores de software
    useEffect(() => {
      const loadSuppliers = async () => {
        try {
          setIsLoading(true);
          const data = await fetchSuppliers("software");
          setSuppliers(data);
        } catch (error) {
          console.error("Error al cargar proveedores de software:", error);
        } finally {
          setIsLoading(false);
        }
      };
  ...
  return (
    ...
    >
      ...
      >
        {/* Listado de proveedores de software - Ocupa 60% del espacio */}
        <div
          style={{
            display: "flex",
            flexDirection: "column",
