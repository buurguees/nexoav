...
/**
 * Página de Cashflow - Versión Desktop (> 1024px)
...
export function CashflowDesktop() {
  ...
  return (
    ...
    >
      ...
      >
        {/* Listado de movimientos de cashflow */}
        <div
          style={{
            display: "flex",
            flexDirection: "column",
        ...
        >
          ...
          >
            <SpaceBlock
              ...
              description="Filtros: fecha, tipo, cuenta, importe, etc."
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Título: Cashflow"
              height="100%"
              color="var(--background-secondary)"
            ...
            <SpaceBlock
              ...
              description="Herramientas: exportar, acciones masivas, etc."
              fontSize={currentConfig.fontSize}
            />
          </div>
          <div style={{ flexShrink: 0, minHeight: currentConfig.tableHeaderHeight }}>
            <SpaceBlock
              ...
              description="Cabecera: fecha, concepto, tipo, importe, cuenta, saldo, etc."
              fontSize={currentConfig.fontSize}
            />
          </div>
          <div
          ...
          >
            <SpaceBlock
              label="Listado de Movimientos Cashflow"
              height="100%"
              color="rgba(0, 200, 117, 0.1)"
              description="Tabla/Lista con información de cada movimiento de cashflow"
              fontSize={currentConfig.fontSize}
            />
          </div>
        </div>
        ...
        >
          ...
          >
            <SpaceBlock
              label="Tarjeta 1: Saldo Actual"
              height="100%"
              color="rgba(0, 200, 117, 0.2)"
              description="Saldo actual total. Indicador de tendencia."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Tarjeta 2: Ingresos del Mes"
              height="100%"
              color="rgba(0, 200, 117, 0.2)"
              description="Total de ingresos del mes. Comparación con mes anterior."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Tarjeta 3: Gastos del Mes"
              height="100%"
              color="rgba(220, 53, 69, 0.2)"
              description="Total de gastos del mes. Comparación con mes anterior."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Tarjeta 4: Flujo Neto"
              height="100%"
              color="rgba(67, 83, 255, 0.2)"
              description="Flujo neto (ingresos - gastos). Indicador positivo/negativo."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Tarjeta 5: Proyección"
              height="100%"
              color="rgba(255, 165, 0, 0.2)"
              description="Proyección de saldo futuro basado en tendencias."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
          ...
          >
            ...
            >
              <div style={{ width: "100%", flex: "1 1 50%", minHeight: 0 }}>
                <SpaceBlock
                  label="Gráfico Barras: Ingresos vs Gastos"
                  height="100%"
                  color="rgba(67, 83, 255, 0.15)"
              ...
              <div style={{ width: "100%", flex: "1 1 50%", minHeight: 0 }}>
                <SpaceBlock
                  label="Gráfico Líneas: Evolución Saldo"
                  height="100%"
                  color="rgba(0, 200, 117, 0.15)"
                  description="Evolución del saldo en el tiempo"
                  fontSize={currentConfig.fontSize}
                />
              </div>
            </div>
            <div style={{ width: "100%", height: "100%", minHeight: 0, display: "flex", flexDirection: "column" }}>
              <SpaceBlock
                label="Pie Chart: Distribución por Tipo"
                height="100%"
                color="rgba(156, 81, 224, 0.15)"
                description="Distribución porcentual por tipo de movimiento"
                fontSize={currentConfig.fontSize}
              />
            </div>
          </div>
