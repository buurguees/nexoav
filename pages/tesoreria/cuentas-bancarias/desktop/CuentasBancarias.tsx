...
/**
 * Página de Cuentas Bancarias - Versión Desktop (> 1024px)
...
export function CuentasBancariasDesktop() {
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <SpaceBlock
              ...
              description="Filtros: banco, tipo, saldo, estado, etc."
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Título: Cuentas Bancarias"
              height="100%"
              color="var(--background-secondary)"
          ...
          <div style={{ flexShrink: 0, minHeight: currentConfig.tableHeaderHeight }}>
            <SpaceBlock
              ...
              description="Cabecera: banco, número, tipo, saldo, estado, etc."
              fontSize={currentConfig.fontSize}
            />
          </div>
          <div
          ...
          >
            <SpaceBlock
              label="Listado de Cuentas Bancarias"
              height="100%"
              color="rgba(0, 200, 117, 0.1)"
              description="Tabla/Lista con información de cada cuenta bancaria"
              fontSize={currentConfig.fontSize}
            />
          </div>
        </div>
        ...
        >
          ...
          >
            <SpaceBlock
              label="Tarjeta 1: Total Cuentas"
              height="100%"
              color="rgba(0, 200, 117, 0.2)"
              description="Total de cuentas bancarias. Indicador de tendencia."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Tarjeta 2: Saldo Total"
              height="100%"
              color="rgba(67, 83, 255, 0.2)"
              description="Saldo total de todas las cuentas. Comparación con período anterior."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Tarjeta 3: Cuentas Activas"
              height="100%"
              color="rgba(0, 200, 117, 0.2)"
            ...
            />
            <SpaceBlock
              label="Tarjeta 4: Saldo Promedio"
              height="100%"
              color="rgba(255, 165, 0, 0.2)"
              description="Saldo promedio por cuenta."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
              label="Tarjeta 5: Banco Principal"
              height="100%"
              color="rgba(156, 81, 224, 0.2)"
              description="Banco con más cuentas asociadas."
              borderWidth="2px"
              fontSize={currentConfig.fontSize}
            />
            <SpaceBlock
          ...
          >
            ...
            >
              <div style={{ width: "100%", flex: "1 1 50%", minHeight: 0 }}>
                <SpaceBlock
                  label="Gráfico Barras: Saldo por Banco"
                  height="100%"
                  color="rgba(67, 83, 255, 0.15)"
                  description="Distribución de saldo por banco"
                  fontSize={currentConfig.fontSize}
                />
              </div>
              <div style={{ width: "100%", flex: "1 1 50%", minHeight: 0 }}>
                <SpaceBlock
                  ...
                  height="100%"
                  color="rgba(0, 200, 117, 0.15)"
                  description="Evolución del saldo total en el tiempo"
                  fontSize={currentConfig.fontSize}
                />
              </div>
            </div>
            <div style={{ width: "100%", height: "100%", minHeight: 0, display: "flex", flexDirection: "column" }}>
              <SpaceBlock
                label="Pie Chart: Distribución por Banco"
                height="100%"
                color="rgba(156, 81, 224, 0.15)"
                description="Distribución porcentual por banco"
                fontSize={currentConfig.fontSize}
              />
            </div>
          </div>
