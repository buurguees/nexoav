...
import { createCompanyBank, CompanyBankData } from "../../../../lib/mocks/companyMocks";
...
// Función para validar IBAN (formato básico)
function validateIBAN(iban: string): boolean {
  if (!iban) return false;
  // Formato básico: ES seguido de 22 dígitos (con o sin espacios)
...
export function NewBankAccountModal({ isOpen, onClose, onSave }: NewBankAccountModalProps) {
  const [formData, setFormData] = useState({
    bank_name: "",
    iban: "",
    swift_bic: "",
    is_visible_on_invoices: true,
    is_default: false,
  });
  ...
  const validate = (): boolean => {
    ...
    if (!formData.iban.trim()) {
      ...
      newErrors.iban = "Formato de IBAN inválido (ej: ES91 2100 0418 4502 0005 1332)";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const created = await createCompanyBank({
      ...
      setErrors({
        submit: "Error al crear la cuenta bancaria. Por favor, inténtalo de nuevo.",
      });
    } finally {
      ...
      >
        ...
        >
          <div>
            ...
            >
              Añadir una nueva cuenta bancaria de la empresa
            </p>
          </div>
          <button
            onClick={onClose}
