...
/**
 * Página de Informes - Versión Tablet Portrait (768px - 1024px)
 ...
 * - Listado de Informes (con scroll)
...
export function InformesTablet() {
  ...
  return (
    ...
    >
      ...
      >
        <SpaceBlock label="Filtros" height="100%" color="rgba(255, 165, 0, 0.15)" description="Filtros de búsqueda: período, tipo, categoría, etc." fontSize={currentConfig.fontSize} />
        <SpaceBlock label="Informes" height="100%" color="var(--background-secondary)" description="Título de la sección de informes" fontSize={currentConfig.fontSize} />
        <SpaceBlock label="Herramientas" height="100%" color="rgba(67, 83, 255, 0.15)" description="Herramientas: crear informe, exportar, acciones masivas, etc." fontSize={currentConfig.fontSize} />
      </div>
      ...
      >
        <SpaceBlock
          label="Tarjeta 1: Total Informes"
          height="100%"
          color="rgba(0, 200, 117, 0.2)"
          description="Total de informes generados. Indicador de actividad."
          borderWidth="2px"
          fontSize={currentConfig.fontSize}
        />
        <SpaceBlock
          label="Tarjeta 2: Informes del Mes"
          height="100%"
          color="rgba(67, 83, 255, 0.2)"
          description="Informes generados en el mes actual. Comparación con mes anterior."
          borderWidth="2px"
          fontSize={currentConfig.fontSize}
        />
        <SpaceBlock
          label="Tarjeta 3: Por Tipo"
          height="100%"
          color="rgba(255, 165, 0, 0.2)"
          description="Distribución de informes por tipo principal."
          borderWidth="2px"
          fontSize={currentConfig.fontSize}
        />
      </div>
      ...
      >
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: "auto", overflowX: "hidden" }}>
          <SpaceBlock
            ...
            description="Tabla/Lista con información de cada informe (código, nombre, tipo, fecha, estado, etc.)"
            fontSize={currentConfig.fontSize}
          />
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <SpaceBlock
              label="Gráfico Barras: Informes por Tipo"
              height="100%"
              color="rgba(67, 83, 255, 0.15)"
              description="Distribución de informes por tipo en el período seleccionado."
              fontSize={currentConfig.fontSize}
            />
          </div>
          ...
          >
            <SpaceBlock
              ...
              description="Evolución de informes a lo largo del tiempo."
              fontSize={currentConfig.fontSize}
            />
          </div>
        </div>
        ...
        >
          <SpaceBlock
            label="Pie Chart: Distribución de Informes"
            height="100%"
            color="rgba(156, 81, 224, 0.15)"
            description="Distribución porcentual de informes por tipo."
            fontSize={currentConfig.fontSize}
          />
        </div>
      </div>
