...
import { fetchFacturaById, updateSalesDocument, SalesDocumentData } from "../../../../lib/mocks/salesDocumentsMocks";
import {
...
interface EditFacturaModalProps {
  ...
  onClose: () => void;
  facturaId: string;
  onSave: (factura: SalesDocumentData) => Promise<void>;
}
...
export function EditFacturaModal({ isOpen, onClose, facturaId, onSave }: EditFacturaModalProps) {
    ...
    if (isOpen && facturaId) {
      const loadData = async () => {
        ...
        try {
          ...
          if (!doc) {
            setErrors({ submit: "Factura no encontrada" });
            return;
          }

          if (doc.status === "cobrada") {
            setErrors({ submit: "No se puede editar una factura cobrada" });
            return;
          }
          ...
          setErrors({ submit: "Error al cargar la factura" });
        } finally {
          setIsLoading(false);
        }
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!factura || !validateStep(4)) return;
    ...
    try {
      ...
      const updatedDoc = await updateSalesDocument(factura.id, {
      ...
      await deleteSalesDocumentLines(factura.id);
      ...
      setErrors({ submit: "Error al actualizar la factura. Por favor, inténtalo de nuevo." });
    } finally {
        ...
        <>
          ...
          >
            ...
            >
              ...
              >
                Editar Factura {factura.document_number}
              </h2>
              <button
                onClick={onClose}
                style={{
