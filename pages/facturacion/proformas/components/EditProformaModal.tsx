...
import { fetchProformaById, updateSalesDocument, SalesDocumentData } from "../../../../lib/mocks/salesDocumentsMocks";
import {
  fetchSalesDocumentLines,
  deleteSalesDocumentLines,
...
interface EditProformaModalProps {
  ...
  onClose: () => void;
  proformaId: string;
  onSave: (proforma: SalesDocumentData) => Promise<void>;
}
...
export function EditProformaModal({ isOpen, onClose, proformaId, onSave }: EditProformaModalProps) {
  ...
  // Cargar proforma y datos relacionados
  useEffect(() => {
    if (isOpen && proformaId) {
      const loadData = async () => {
        ...
        try {
          ...
          if (!doc) {
            setErrors({ submit: "Proforma no encontrada" });
            return;
          }

          // Solo editable si está en borrador o enviada
          if (doc.status !== "borrador" && doc.status !== "enviado") {
            setErrors({ submit: "Solo se pueden editar proformas en estado 'borrador' o 'enviado'" });
            return;
          }
          ...
          setErrors({ submit: "Error al cargar la proforma" });
        } finally {
          setIsLoading(false);
        }
      };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!proforma || !validateStep(4)) return;
    ...
    try {
      ...
      const updatedDoc = await updateSalesDocument(proforma.id, {
      ...
      if (!updatedDoc) {
        throw new Error("Error al actualizar la proforma");
      }

      await deleteSalesDocumentLines(proforma.id);
      ...
      setErrors({ submit: "Error al actualizar la proforma. Por favor, inténtalo de nuevo." });
    } finally {
        ...
        <>
          ...
          >
            ...
            >
              ...
              >
                Editar Proforma {proforma.document_number}
              </h2>
              <button
                onClick={onClose}
                style={{
              ...
              <div style={{ padding: "var(--spacing-lg)", flex: 1 }}>
                ...
                {currentStep === 2 && (
                  <div>
                    ...
                    <div
                      style={{ display: "flex", gap: "var(--spacing-md)", marginTop: "var(--spacing-md)" }}
                    >
                      <div style={{ flex: 1 }}>
                        <label
                          style={{
                ...
                {currentStep === 3 && (
                  <div>
                    <PresupuestoLinesEditor lines={lines} onChange={setLines} defaultTaxPercent={21} />
                    {errors.lines && (
                      <span
                        style={{
                          display: "block",
                  ...
                  <div>
                    ...
                    <div style={{ marginBottom: "var(--spacing-md)" }}>
                      ...
                      <textarea
                        ...
                        placeholder="Observaciones que aparecerán en el PDF de la proforma"
                      />
                    </div>
                    <div>
                      <label
