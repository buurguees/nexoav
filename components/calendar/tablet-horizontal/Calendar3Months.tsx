...
import { TaskCategoryDot } from "../../tasks/tablet-horizontal/TaskCategoryDot";
...
export function Calendar3Months({
  ...
  // Calcular la fecha de inicio (hoy) y fin (1 mes desde hoy = 2 meses en total: hoy + 1 siguiente)
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  
  // Calcular el último día del mes que está 1 mes después de hoy (2 meses en total)
  const twoMonthsLater = new Date(today.getFullYear(), today.getMonth() + 2, 0); // Último día del mes siguiente
  twoMonthsLater.setHours(23, 59, 59, 999);
  
  // Estado para el mes actual visible (permite navegación)
  ...
  React.useEffect(() => {
    ...
    const cells = calendarContainer.querySelectorAll('.calendar-cell-tablet-horizontal');
    
    cells.forEach((cell) => {
      // Limpiar puntos anteriores
      const existingDots = cell.querySelectorAll('.task-dot-container-tablet-horizontal');
      ...
      if (!dayDate || isNaN(dayDate.getTime())) {
        ...
        const monthElement = cell.closest('.calendar-month-tablet-horizontal');
        if (!monthElement) return;
        
        // Obtener el texto del caption del mes (ej: "noviembre 2025")
        const captionLabel = monthElement.querySelector('.calendar-caption-label-tablet-horizontal');
        ...
        const isOutside = dayButton.classList.contains('day-outside-tablet-horizontal') || dayButton.classList.contains('day-outside');
      ...
      // No restringir por rango de fechas (permite navegación libre)
      ...
      // Crear contenedor para los puntos - TABLET HORIZONTAL: igual que tablet portrait
      const dotsContainer = document.createElement('div');
      dotsContainer.className = 'task-dot-container-tablet-horizontal';
      ...
      // TABLET HORIZONTAL: igual que tablet portrait (5px)
      ...
      const dotElements = Array.from(taskTypes).map((taskType) =>
        React.createElement(TaskCategoryDot, {
          key: taskType,
          type: taskType,
          size: 5, // TABLET HORIZONTAL: igual que tablet portrait
        })
      );
      root.render(
    ...
    return () => {
      ...
      const containers = calendarContainer.querySelectorAll('.task-dot-container-tablet-horizontal');
      containers.forEach(container => container.remove());
    };
  }, [date, tasks, currentMonth]);

  return (
    <div ref={calendarRef} className={cn("calendar-container-tablet-horizontal w-full", className)} style={{ position: "relative" }}>
      <Calendar
        mode="single"
        selected={date}
        ...
        className="w-full"
        classNames={{
          months: "calendar-months-tablet-horizontal",
          month: "calendar-month-tablet-horizontal",
          caption: "calendar-caption-tablet-horizontal",
          caption_label: "calendar-caption-label-tablet-horizontal",
          nav: "calendar-nav-tablet-horizontal",
          nav_button: "calendar-nav-button-tablet-horizontal",
          nav_button_previous: "calendar-nav-button-tablet-horizontal calendar-nav-button-previous-tablet-horizontal",
          nav_button_next: "calendar-nav-button-tablet-horizontal calendar-nav-button-next-tablet-horizontal",
          table: "calendar-table-tablet-horizontal",
          head_row: "calendar-head-row-tablet-horizontal",
          head_cell: "calendar-head-cell-tablet-horizontal",
          row: "calendar-row-tablet-horizontal",
          cell: "calendar-cell-tablet-horizontal",
          day: "calendar-day-tablet-horizontal",
          day_range_start: "calendar-day-tablet-horizontal day-range-start-tablet-horizontal",
          day_range_end: "calendar-day-tablet-horizontal day-range-end-tablet-horizontal",
          day_selected: "calendar-day-tablet-horizontal",
          day_today: "calendar-day-tablet-horizontal day-today-tablet-horizontal",
          day_outside: "calendar-day-tablet-horizontal day-outside-tablet-horizontal",
          day_disabled: "calendar-day-tablet-horizontal",
          day_range_middle: "calendar-day-tablet-horizontal",
          day_hidden: "calendar-day-hidden-tablet-horizontal",
        }}
      />
      <DayPopup
        open={isPopupOpen}
